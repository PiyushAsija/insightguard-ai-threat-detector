...
const limiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 20,
  message: { error: 'Too many requests. Please wait 15 minutes.', code: 'RATE_LIMITED' }
});
app.use('/api/', limiter);
...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    const mimeOk = ['text/csv', 'application/vnd.ms-excel', 'text/plain'].includes(file.mimetype);
...
app.get('/health', (req, res) => {
  const keySet = !!process.env.GROQ_API_KEY && process.env.GROQ_API_KEY !== 'your-groq-api-key-here';
  res.json({
    status: 'ok',
...
app.get('/test-api', async (req, res) => {
  ...
  try {
    const completion = await groq.chat.completions.create({
      ...
      messages: [{ role: 'user', content: 'Reply with only this JSON: {"status":"working","message":"Groq API connected"}' }],
...
app.post('/api/analyze', upload.single('logfile'), async (req, res) => {
  ...
  if (!process.env.GROQ_API_KEY || process.env.GROQ_API_KEY === 'your-groq-api-key-here') {
    return res.status(500).json({
      error: 'GROQ_API_KEY not set. Open backend/.env and add your key from console.groq.com',
      code: 'NO_API_KEY'
    });
  }
  ...
  try {
    ...
    rows = result.data;
    if (rows.length === 0) return res.status(400).json({ error: 'CSV file is empty.', code: 'EMPTY_FILE' });
    if (rows.length > 5000) return res.status(400).json({ error: 'Max 5,000 rows allowed.', code: 'TOO_MANY_ROWS' });

    const required = ['username', 'timestamp', 'action', 'resource', 'ip_address'];
    const found = Object.keys(rows[0]);
    const missing = required.filter(c => !found.includes(c));
    if (missing.length > 0) {
      return res.status(400).json({ error: `Missing columns: ${missing.join(', ')}`, code: 'MISSING_COLUMNS', expected: required, found });
    }
  } catch (err) {
    return res.status(400).json({ error: 'Failed to parse CSV file.', code: 'PARSE_FAILED' });
  }
...
Return a JSON object with this exact structure:
{
  ...
  "anomalies": [
    ...
    try {
      ...
      console.error('❌ JSON parse failed:', completion.choices[0].message.content.substring(0, 300));
      return res.status(500).json({ error: 'AI returned invalid format. Please try again.', code: 'AI_PARSE_ERROR' });
    }

    if (!analysisData.anomalies || !Array.isArray(analysisData.anomalies)) {
      return res.status(500).json({ error: 'AI response missing required fields.', code: 'INVALID_AI_RESPONSE' });
    }
    ...
    console.log(`✅ [${new Date().toISOString()}] ${rows.length} rows → ${analysisData.anomalies.length} anomalies found`);
    ...
    if (err.message?.includes('401') || err.message?.includes('invalid_api_key')) {
      return res.status(500).json({ error: 'Invalid Groq API key. Get one free at console.groq.com', code: 'INVALID_API_KEY' });
    }
    if (err.message?.includes('429') || err.message?.includes('rate_limit')) {
      return res.status(429).json({ error: 'Rate limit hit. Groq free tier allows 30 requests/min. Wait a moment and retry.', code: 'RATE_LIMITED' });
    }
...
app.use((err, req, res, next) => {
  if (err instanceof multer.MulterError) {
    if (err.code === 'LIMIT_FILE_SIZE') return res.status(400).json({ error: 'File too large. Max 2MB.', code: 'FILE_TOO_LARGE' });
    return res.status(400).json({ error: err.message, code: err.code });
  }
  if (err.message === 'INVALID_FILE_TYPE') return res.status(400).json({ error: 'Only CSV files accepted.', code: 'INVALID_FILE_TYPE' });
